...
class Experiment:
    ...
    def improvement_iterations(self):
        return {
            ...
            if self.monitor_list_callback_monitor_best[i]
            != self.monitor_list_callback_monitor_best[i + 1]
        }

    def last_improvement_iteration(self):
        return sum(
            np.array(self.step3_monitor_iter_best_fx)
            != self.step3_monitor_iter_best_fx[-1]
        )
    ...
    def step3_num_thetas(self):
        return (
            None
            if self.step3_monitor_iter_thetas is None
            else len(self.step3_monitor_iter_thetas[0])
        )
    ...
    def step4_x_diff(self):
        return (
            None
            if self.step4_result_best_x is None
            else sum(np.abs(self.refx - self.step4_result_best_x))
        )
    ...
    def step4_x_hamming_weight(self):
        return (
            None
            if self.step4_result_best_x is None
            else sum(np.abs(self.step4_result_best_x))
        )

    def step4_rel_gap(self):
        return (
            None
            if self.step4_result_best_fx is None
            else (self.step4_result_best_fx - self.refvalue) / self.refvalue
        )
    ...
    def read_experiments(experiment_set="1/31bonds/") -> list[dict]:
        pathlist = Path(f"{ROOT}/data/{experiment_set}").glob("**/exp*.pkl")
