...
def run_step4(experiment_key, search_type='unconstrained', root_override=None):
    """
    Modular local search postprocessing for a given experiment.
    """
    ROOT = Path(root_override) if root_override else Path.cwd().parent
    doe = doe_localsearch[search_type]
    experiments = Experiment.read_experiments(experiment_key)
    for xp in experiments:
        if xp.has_step4():
            continue
        # Ensure path normalization for any user
        lp_file = xp.lp_file
        # Normalize away legacy hardcoded paths
        if '/home/gabriele-agliardi/data/Client-Vanguard-Optimization' in lp_file:
            lp_file = lp_file.replace('/home/gabriele-agliardi/data/Client-Vanguard-Optimization', str(ROOT))
        if '/Users/andrypaez/projects/WISER_Optimization_VG' in lp_file:
            lp_file = lp_file.replace('/Users/andrypaez/projects/WISER_Optimization_VG', str(ROOT))
        ...
        with open(out_file, 'bw') as f:  # placeholder file
            pkl.dump({}, f)
        model = docplex.mp.model_reader.ModelReader.read(lp_file.replace('.lp','-nocplexvars.lp'))
