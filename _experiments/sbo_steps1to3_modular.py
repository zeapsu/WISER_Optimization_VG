...
def warm_start_nft(
    ...
    obj_fn, ansatz_, _, backend, initial_layout = problem_mapping(
        lp_file.replace(".lp", "-nocplexvars.lp"),
        ansatz,
        ansatz_params,
        theta_initial,
        device,
    ...
    he = HardwareExecutor(
        ...
        },  # TODO: expose the entire sampler_options?
        use_session=False,
        ...
        solver_options={
            ...
            "idx_set_start": last_iter_info["optimizer_internal_status_dict"][
                "idx_set"
            ],
            "step_start": last_iter_info["optimizer_internal_status_dict"][
                "iter_in_epoch"
            ]
            + 1,
        },
    ...
    he.optimization_monitor.list_callback_monitor_best = data[
    ...
    out = Experiment.from_step3(
        ...
        refx,
        refval,
        Experiment.get_current_classical_hw(),
        (data["step3_time"] + step3_time) if data["step3_time"] is not None else None,
        data["step3_job_ids"] + he.job_ids,
        result,
        he.optimization_monitor,
    )
    ...
    for exec in range(num_exec):
        ...
        if not run_on_serverless:
            ...
            he = HardwareExecutor(
                ...
                use_session=False,
                iter_file_path_prefix=str(
                    Path(lp_file).parent / experiment_id_with_exec
                ),
            ...
            remote_step3 = next(
                ...
                if program.title == "execute_on_hw"
            )

            with open(
                lp_file.replace(".lp", "-nocplexvars.lp"), "r", encoding="utf-8"
            ) as file:
...
def auto_resume(experiment_dir: Path):
    ...
    final_files = sorted(
        f for f in experiment_dir.glob("exp*.pkl") if "_" not in f.name
    )
    ...
    if not incomplete_runs:
        raise RuntimeError(
            "No incomplete experiments found (all exp{i}.pkl files are present)."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run quantum optimization experiments."
    )
    parser.add_argument(
        "--experiment", type=str, help="Key from the `doe` dictionary to run."
    )
    parser.add_argument(
        "--resume", action="store_true", help="Resume an incomplete experiment."
    )
    parser.add_argument(
    ...
    if args.resume:
        ...
        execute_multiple_runs(
            **doe[args.experiment], instance="", run_on_serverless=False
        )
